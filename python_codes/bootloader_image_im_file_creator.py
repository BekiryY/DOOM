...
image = cv2.imread("boot_img.jpeg")
if image is None:
...
#Address_depth=16384
#Data_width=32\n""")
    # VGA Controller sub_pixel mapping:
    #   sub_pixel=0 -> bits [7:0]   -> p0 (LSByte)
    #   sub_pixel=1 -> bits [15:8]  -> p1
    #   sub_pixel=2 -> bits [23:16] -> p2
    #   sub_pixel=3 -> bits [31:24] -> p3 (MSByte)
    # So the 32-bit word must be packed as: {p3, p2, p1, p0}
    ...
    for i in range(0, len(data_flattened_hex), 4):
        ...
        # Pack p3 as MSByte, p0 as LSByte to match VGA sub_pixel case statement
        packed_word = f"{p3}{p2}{p1}{p0}"
        f.write(packed_word + "\n")
        word_count += 1
